...
public class OkHttpClientWrap {
    private static final String TAG = "OkHttpClientWrap";
    ...
    public static OkHttpClientWrap instance() {
        for (; ; ) {
            OkHttpClientWrap instance = INSTANCE.get();
    ...
    private OkHttpClientWrap() {
        ...
        USER_AGENT = mApplication.getPackageName() + "/" + PackageManagerUtil.getVersionName();
        ...
        Logg.v(TAG, "created OkHttpClientWrap instance!");
    }

    /**
     * Application onCreate调用.   初始化SSL,支持双向证书认证(需要配置相关信息).
     */
    public void initSSLAccess() {
        if (TextUtils.isEmpty(CERT_CA_NAME) || TextUtils.isEmpty(CERT_CLIENT_PFX_NAME) || TextUtils.isEmpty(CERT_PASSWORD)) {
            instance().initFreeAllAccess();
        } else {
            try {
                new HttpsProxy().setCertificates(new InputStream[]{mApplication.getAssets().open(CERT_CA_NAME)},
                        mApplication.getAssets().open(CERT_CLIENT_PFX_NAME), CERT_PASSWORD);
            } catch (Exception e) {
                Logg.e(TAG, "IOException : " + e.getMessage(), e);
            }
        }
    }
    ...
    private class GetProxy {
        ...
        void get(String url, final IRequestCallback callback, Object tag) {
    ...
    private class PostProxy {
        ...
        void post(String url, Map<String, String> params, final IRequestCallback callback, Object tag) {
    ...
    private class UploadProxy {
        ...
        private Request buildUploadRequest(String url, Map<String, String> paramMap, Map<String, File> fileMap, final IRequestCallback callback, Object tag) {
            ...
            ProgressRequestBody progressRequestBody = new ProgressRequestBody(builder.build(),
                    (writedSize, totalSize, completed) -> sendProgress2UICallback(callback, writedSize, totalSize, completed));
    ...
    private void deliveryResult(final IRequestCallback callback, Request request) {
        ...
        mBuilder.build().newCall(request).enqueue(new Callback() {
            @Override
        ...
        try {
            ...
            clone.networkInterceptors().add(chain -> {
                ...
                return originalResponse.newBuilder().body(new ProgressResponseBody(originalResponse.body(),
                        (writedSize, totalSize, completed) -> sendProgress2UICallback(callback, writedSize, totalSize, completed))).build();
            });
