package com.simuwang.xxx.manager.cache;
...
public class CacheManager {
    ...
    private CacheManager() {
        dirExternal = new File(FileStorageUtil.getAppCacheDirPath());
        dirData = new File(MyApp.getApplication().getCacheDir().getAbsolutePath());
        isCacheOpened();
    }
    ...
    private boolean isCacheOpened() {
        ...
        try {
            mDiskLruCache = DiskLruCache.open(
                    FileStorageUtil.getAppCacheDir(),
                    CacheUtil.getAppVersion(MyApp.getApplication()),
    ...
    private void delete() {
        ...
        try {
            ...
            FileStorageUtil.deleteDirsAndFile(dirExternal);
            FileStorageUtil.deleteDirsAndFile(dirData);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    private long sizeOf() {
        ...
        flush();
        return FileStorageUtil.sizeFiles(dirExternal) + FileStorageUtil.sizeFiles(dirData);
    }
    ...
    public DiskLruCache.Editor editor(String key) {
        ...
        try {
            ...
            if (edit == null) {
                Logg.w(TAG, "the entry spcified key:" + key + " is editing by other . ");
            }
            return edit;
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public InputStream get(String key) {
        ...
        try {
            ...
            if (snapshot == null) {//not find entry , or entry.readable = false
                Logg.e(TAG, "not find entry , or entry.readable = false");
                return null;
            }
            //write READ
